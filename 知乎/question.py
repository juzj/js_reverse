...
function O(e, t, n) {
    ...
    data = ctx.call(
        "O", f'3_2.0+{url}+"AIBarl-nzxKPTirxmt9-GdomTcOdsa9UAW4=|1615953056"',
    )
    return data
...
var encrypt = function(e) {
    ...
    data = ctx.call("encrypt", data)
    return data


def get_username(url, zse):
    headers = {
        ...
        "cookie": 'd_c0="AIBarl-nzxKPTirxmt9-GdomTcOdsa9UAW4=|1615953056";',
    }

    response = requests.get(f"https://www.zhihu.com{url}", headers=headers,)
    for info in response.json()["data"]:
        data = {
            "title": info["question"]["title"],  # 标题
            "created": info["question"]["created"],  # 问题创建时间
            "updated_time": info["question"]["updated_time"],  # 评论时间
            "com_one": info["question"]["id"],  # 采集标签ID
            "com_two": info["id"],  # 采集标签ID
            "comment_count": info["comment_count"],  # 评论条数
            "voteup_count": info["voteup_count"],  # 赞同条数
            "excerpt": info["excerpt"].replace("[图片]", "").strip(),  # 评论内容
        }
        print(data)


if __name__ == "__main__":
    url = f"/api/v4/members/he-xie-hou-chen-mo/answers?include=data%5B*%5D.is_normal%2Cadmin_closed_comment%2Creward_info%2Cis_collapsed%2Cannotation_action%2Cannotation_detail%2Ccollapse_reason%2Ccollapsed_by%2Csuggest_edit%2Ccomment_count%2Ccan_comment%2Ccontent%2Ceditable_content%2Cattachment%2Cvoteup_count%2Creshipment_settings%2Ccomment_permission%2Cmark_infos%2Ccreated_time%2Cupdated_time%2Creview_info%2Cexcerpt%2Cis_labeled%2Clabel_info%2Crelationship.is_authorized%2Cvoting%2Cis_author%2Cis_thanked%2Cis_nothelp%2Cis_recognized%3Bdata%5B*%5D.vessay_info%3Bdata%5B*%5D.author.badge%5B%3F(type%3Dbest_answerer)%5D.topics%3Bdata%5B*%5D.question.has_publishing_draft%2Crelationship&offset=0&limit=20&sort_by=created"
    zse = encrypt(url_encode(url))
    get_username(url, zse)
